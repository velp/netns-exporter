...
// NewMagnumV1Clients returns reference to instances of the Magnum V1 service
...
func NewMagnumV1Clients(opts *common.NewClientOpts) (map[string]gophercloud.ServiceClient, error) {
	regions := config.Config.OpenStack.Magnum.Regions
	clients := make(map[string]gophercloud.ServiceClient, len(regions))

	for _, region := range regions {
		client, err := openstack.NewContainerInfraV1(opts.Provider, gophercloud.EndpointOpts{
			Region:       region,
			Availability: gophercloud.Availability(opts.EndpointType),
		})
		if err != nil {
			return nil, errors.Wrapf(err, "error initializing Magnum client in %s region", region)
		}
