...
// NewNeutronV2Clients returns reference to instances of the Neutron V2 service
...
func NewNeutronV2Clients(opts *common.NewClientOpts) (map[string]gophercloud.ServiceClient, error) {
	regions := config.Config.OpenStack.Neutron.Regions
	clients := make(map[string]gophercloud.ServiceClient, len(regions))

	for _, region := range regions {
		client, err := openstack.NewNetworkV2(opts.Provider, gophercloud.EndpointOpts{
			Region:       region,
			Availability: gophercloud.Availability(opts.EndpointType),
		})
		if err != nil {
			return nil, errors.Wrapf(err, "error initializing Networking client in %s region", region)
		}
